...
| **Numbering** | What order field IDs run in: top-left to bottom-right, down each column, in equal horizontal chunks, outward in rings from a chosen corner, largest first, or raw detection order. Detection order numbers a field by its single topmost pixel, which is why it looks arbitrary. |
...
| `--numbering <o>` | Field ID order: `rows` / `columns` / `chunks` / `radial` / `area` / `source` | `rows` |
| `--chunks <n>` | Horizontal bands for `--numbering chunks` (2–20) | 4 |
| `--corner <c>` | Corner for `--numbering radial`: `nw` / `ne` / `sw` / `se` | `nw` |
| `--rings <n>` | Rings for `--numbering radial` (2–20) | 6 |
